 ...
 * Provides food value balancing and growth modifications for HarvestCraft items.
...
public class HarvestCraftModule {
    ...
    private FoodValues categorizeItem(String itemName) {
        ...
        // Only match if item name is exactly a crop name (not a longer name containing it)
    ...
    private String getCategoryName(String itemName) {
        if (containsAny(itemName, COMPLEX_MEALS)) return "COMPLEX_MEAL";
        if (containsAny(itemName, PIZZAS)) return "PIZZA";
        if (containsAny(itemName, BURGERS)) return "BURGER";
        if (containsAny(itemName, SANDWICHES)) return "SANDWICH";
        if (containsAny(itemName, SOUPS_STEWS)) return "SOUP_STEW";
        if (containsAny(itemName, PIES_DESSERTS)) return "PIE_DESSERT";
        if (containsAny(itemName, SMOOTHIES)) return "SMOOTHIE";
        if (containsAny(itemName, YOGURTS)) return "YOGURT";
        if (containsAny(itemName, ICE_CREAM)) return "ICE_CREAM";
        if (containsAny(itemName, JAMS)) return "JAM";
        if (containsAny(itemName, BASIC_COOKED)) return "BASIC_COOKED";
        if (containsAny(itemName, SIMPLE_PROCESSED)) return "SIMPLE_PROCESSED";
        if (RAW_MEATS.contains(itemName)) return "RAW_MEAT";
        if (CONDIMENTS.contains(itemName)) return "CONDIMENT";
        if (containsAny(itemName, SNACKS)) return "SNACK";
        if (containsAny(itemName, CANDIES)) return "CANDY";
        if (RAW_CROPS.contains(itemName)) return "RAW_CROP";
        return "UNKNOWN";
    }
